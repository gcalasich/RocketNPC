...
pub fn authenticate(_login: authenticate::Login)-> Result<Json<api_response::ApiResponse>, APIError>{  
    ...
    if _login.userName == "admin" && _login.userPassword == "admin" {
        ...
        let response :api_response::ApiResponse = api_response::ApiResponse::new(String::from(""), token, String::from(""), 0);
        return Ok(Json(response))
    }
    
    return  Err(APIError::new(api_error::APIErrorTypes::AuthenticationError, format!("{}","Invalid Password or Username"), FILE.to_string(),METHOD.to_string(), Utc::now(), format!("pasword = {}", "none".to_owned()), api_error::APIErrorCodes::APPAUTAUT05))
