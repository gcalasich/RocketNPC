...
pub fn commit(commit: &commit::Commit)-> Result<Json<api_response::ApiResponse>, APIError>{  
    ...
    let transaction_id = commit.transactionId;
    
    let response :api_response::ApiResponse = api_response::ApiResponse::new(String::from(""), String::from(""), String::from("Successful commit performed"), 0);
    Ok(Json(response))
}