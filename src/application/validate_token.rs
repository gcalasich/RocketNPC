...
pub fn is_token_valid(token: &String) -> Result<bool, APIError>{
    ...
    let session_timeout: i32 = env::var("SESSION_TIMEOUT").expect("Session Time Out Should be set").parse::<i32>().unwrap();
    ...
    let datetime_now_plus_session_time_out_opt = authentication_datetime.checked_add_signed(chrono::Duration::minutes(session_timeout.into()));    
   
    let mut datetime_now_plus_session_time_out = Utc::now();
  
    match datetime_now_plus_session_time_out_opt{
        None => {
            return Err(APIError::new(api_error::APIErrorTypes::GeneralException, "Failed to add session time_out to last authentication time".to_owned(), FILE.to_string(), METHOD.to_string(), Utc::now(), format!("token= {}", token.to_owned()) , api_error::APIErrorCodes::APPVTOVTO01))
        }  
...
pub fn validate_token(api_params: &String, token: &String)-> Result<(),APIError>{
    const METHOD : &str = "validate_token";  
    let is_token_valid_result = is_token_valid(&token);
    if let Err(e) = is_token_valid_result{
        //return Err(error_handler::handle_error_struct(e, &String::from("Failed Validating Token"), &String::from("API Execution failed.")))
        return Err(APIError::new(api_error::APIErrorTypes::GeneralException, "Failed Validating Token".to_owned(), FILE.to_string(), METHOD.to_string(), Utc::now(), format!("token= {}", token.to_owned()) , api_error::APIErrorCodes::APPVTOVTO02))
    }
    ...
    if is_token_valid == false{
        ...
        return Err(APIError::new(api_error::APIErrorTypes::GeneralException, "Token Expired".to_owned(), FILE.to_string(), METHOD.to_string(), Utc::now(), format!("token= {}", token.to_owned()) , api_error::APIErrorCodes::APPVTOVTO03))
    }
    Ok(())
}
